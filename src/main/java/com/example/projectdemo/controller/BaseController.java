...
import com.example.projectdemo.entity.UuidEntity;
...
public class BaseController {
   ...
   * @return UserEntity
   */
  protected UserEntity verifyLdapNodeLogin(Principal operator) {
    Validate.notNull(operator, "not found operator");
    String account = getPrincipalAccount(operator);
    return userService.findByAccountAndStatus(account, UseStatus.STATUS_NORMAL);
  }
  ...
  private void filterProperty(String property, Object currentObject, Stack<Class<?>> stackClasses)
    ...
    if ((nodeIndex = property.indexOf(".")) != -1) {
      ...
      try {
        ...
        throw new IllegalArgumentException("not found property: " + currentFieldName
            + " in object " + currentObject.getClass().getName());
      }
  ...
  private Field findField(String currentFieldName, Class<?> targetClass)
    ...
    if (currentField == null) {
      ...
      if (superClass != null) {
        ...
        throw new NoSuchFieldException("not found property " + currentFieldName + " in class "
