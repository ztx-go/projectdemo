...
public class SecurityController extends BaseController {
  ...
  public @ApiIgnore
  ResponseModel loginSuccess(HttpServletRequest request , HttpServletResponse response , Principal logUser) {
    String account = logUser.getName();
    if(StringUtils.isEmpty(account)) {
      throw new AccessDeniedException("无法获取该用户!");
    }
    
    // 查询用户基本信息
    UserEntity user = userService.findByAccount(account);
    if(null != user){
      userService.updateLoginTime(account, new Date());
    }
    // 隔断用户关联信息后返回
    return this.buildHttpReslut(user, "roles","createUser","modifyUser","depart");
  }
  ...
  public void loginFail() throws IllegalAccessException {
    throw new IllegalAccessException("url Access Denied or login fail!");
  }
   ...
   */
  @ApiOperation(value="形成角色和后台用户的绑定关系（注意，不能重复绑定）")
  ...
  public void bindRoleAndCompetence(String roleId , String competenceId) {
  ...
  public void bindRoleAndCompetences(String roleId , String[] competenceIds) {
   ...
   * 解除角色和后台用户的绑定关系
   */
  @ApiOperation(value="解除角色和后台用户的绑定关系")
  ...
  public void unbindRoleForOperator(String roleId , String operatorId) {
    this.securityService.unbindRoleForOperator(roleId, operatorId);
  }
  ...
  public void unbindRoleAndCompetence(String roleId , String competenceId) {
  ...
  public void unbindRoleAndCompetences(String roleId , String[] competenceIds) {
