...
public interface SecurityRepository extends JpaRepository<RoleEntity, String> {
  ...
  public void bindRoleForUser(@Param("roleId") String roleId, @Param("userId") String userId);
   ...
   * @param roleId 指定的角色
   * @param operatorId 指定的后台管理员
   */
  @Query("from UserEntity u left join fetch u.roles r where u.id = :userId and r.id = :roleId")
  public UserEntity findUserByRoleIdAndUserId(@Param("roleId") String roleId, @Param("userId") String userId);
  ...
  public void unbindRoleForUser(@Param("roleId") String roleId, @Param("userId") String userId);
